...
本地测试包位于 `dist/Paper-Atlas-1.1.0.dmg`。公开 Release 只接受经过 Developer ID 签名和 Apple 公证的构建；在证书准备好之前不上传 DMG。
...
- 图谱始终只显示当前选中论文的一阶关系，避免完整网络遮挡节点和年份。
...
三个来源会合并去重，已存在于论文库中的同标题论文不会再次进入候选。候选按时间倒序排列；选择候选后可在右侧查看摘要、元数据校验和推荐依据。高被引候选优先根据标题和摘要分类，证据不足时使用命中的搜索主题给出待确认建议。点击“加入论文库”后，Paper Atlas 会依次完成下载、PDF 校验、重复检查、分类归档、决策写入和图谱重建。
