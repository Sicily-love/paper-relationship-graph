...
- 领域高被引按同一组主题搜索 OpenAlex，再按引用量排序；默认要求至少被引 50 次，每个主题最多保留 5 篇。
- 共同引用从现有论文的参考文献中找出被多篇论文共同引用、但尚未归档的论文。

三个来源会合并去重，候选仍按时间倒序排列，并显示来源、摘要、被引次数、相关性证据、元数据检查和推荐分类。点击“加入论文库”后，Paper Atlas 会依次完成下载、PDF 校验、重复检查、分类归档、决策写入和图谱重建。
