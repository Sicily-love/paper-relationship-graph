...
Paper Atlas 是一个本地论文库工具。它按研究方向和年份组织论文，显示库内引用关系，并把 arXiv 搜索与共同引用推荐放在同一个审核流程里。
...
“搜索 arXiv”和“共同引用”是两套独立来源：

- arXiv 搜索按主题获取近期论文，并根据标题、摘要、关键词和领域信号计算相关性。低相关结果不会进入候选列表。
- 共同引用从现有论文的参考文献中找出被多篇论文共同引用、但尚未归档的论文。

候选按时间倒序排列，可以查看摘要、相关性证据、元数据检查和推荐分类。点击“加入论文库”后，Paper Atlas 会依次完成下载、PDF 校验、重复检查、分类归档、决策写入和图谱重建。
...
如果仍保留 Codex 中的同名定时任务，请只启用其中一套调度方式，避免重复执行。
...
检查是否同时启用了 Paper Atlas 的 `launchd` 任务和 Codex 定时任务，只保留一套。
